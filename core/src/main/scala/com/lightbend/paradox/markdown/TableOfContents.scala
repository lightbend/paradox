...
class TableOfContents(
    ...
    maxDepth: Int = 6,
    maxExpandDepth: Option[Int] = None
) {
  ...
  /**
   * Find the headers below the buffer index for a toc directive. Return the level of the next header and sub-headers to
   * render.
   */
  @tailrec
  private def headersBelow(
      location: Option[Location[Header]],
      index: Int,
      includeIndexes: List[Int]
  ): (Int, Forest[Header]) = location match {
    case Some(loc) =>
  ...
  private def isBelow(
      tocIndex: Int,
      tocIncludeIndexes: List[Int],
      headerIndex: Int,
      headerIncludeIndexes: List[Int]
  ): Boolean =
  ...
  private def subList[A <: Linkable](
      base: String,
      active: Option[Location[Page]],
      tree: Tree[A],
      depth: Int,
      expandDepth: Option[Int]
  ): Option[Node] =
    tree.label match {
      case page: Page =>
  ...
  private def item[A <: Linkable](base: String, active: Option[Location[Page]], depth: Int, expandDepth: Option[Int])(
      tree: Tree[A]
  ): Node = {
  ...
  private def autoExpand[A <: Linkable](
      linkable: A,
      active: Option[Location[Page]],
      expandDepth: Option[Int]
  ): Option[Int] =
